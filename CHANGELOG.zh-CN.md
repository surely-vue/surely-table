...
- 🌟 新增整行任意位置发起行拖动：顶层 `rowDrag` prop 接受导出的 `WHOLE_ROW_DRAG_KEY` 常量，开启后整行可拖动；与 `rangeSelection` 互斥，开启时框选自动失效
...
- 🐞 修复导出 Excel 时，未设置 `key` 的列（仅设置 `dataIndex`）在多行表头场景下被跳过，导致表头与数据错位问题
...
- 🌟 新增 customHeaderCheckbox 插槽，用于自定义表头复选框
- 🐞 修复 autoHeight 时，行高度计算错误问题

## 5.0.12 / 5.0.13

- 🐞 修复 autoHeight 时，合并行高度计算错误问题
...
- 🐞 修复 HeaderCellTitle 自定义元素更新渲染问题

## 5.0.8

- 🐞 修复 autoHeight 时，合并行行高度计算错误问题
...
- 🐞 修复 Firefox 低版本浏览器下滚动不到最右侧问题

## 5.0.3

- 🌟 新增 `yBuff` 属性，用于自定义纵向缓冲区
- 🌟 新增 `xBuff` 属性，用于自定义横向缓冲区

## 5.0.2

- 🐞 修复 Tooltip zIndex 问题
